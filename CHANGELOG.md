...
- Windows 支持修改并保存本机局域网 IPv4，同步服务监听所选地址
- 校验 IP 是否属于本机网卡，启动失败时显示错误；停止同步后可修改 IP
- 自动启动使用已保存的 IP；地址失效时显示窗口提示修改

## 1.2.2

- Windows 首次手动开始同步时自动请求管理员授权，创建仅限专用网络的 TCP 45837 入站防火墙规则
- 防火墙规则按端口配置，不绑定可执行文件路径，更新或重新解压后无需再次配置
